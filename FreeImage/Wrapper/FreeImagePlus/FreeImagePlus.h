...
public:
	...
	Format(FREE_IMAGE_FORMAT fif) {
		format = fif;
	}
	Format(const std::string& name) {
		format = FreeImage_GetFIFFromFormat(name.c_str());
	}
	Format(FreeImageIO *io, fi_handle handle) {
		format = FreeImage_GetFileTypeFromHandle(io, handle, 32);
	}
#if 0
	Format(const MemoryIO& memory);
#endif
	...
	Format& operator=(FREE_IMAGE_FORMAT rhs) {
		format = rhs;
		return *this;
	}
	...
	bool operator==(FREE_IMAGE_FORMAT fif) const {
		return format == fif;
	}
	operator FREE_IMAGE_FORMAT() const {
		return format;
	}

	std::string getName() const {
		return std::string(FreeImage_GetFormatFromFIF(format));
	}
	std::string getMime() const {
		return std::string(FreeImage_GetFIFMimeType(format));
}
	std::string getDescription() const {
		return std::string(FreeImage_GetFIFDescription(format));
	}
	std::string getFirstExtension() const;
	std::string getExtensions() const {
		return std::string(FreeImage_GetFIFExtensionList(format));
	}
	bool supportsReading() const {
		return FreeImage_FIFSupportsReading(format) == TRUE;
	}
	...
	bool supportsBPP(int bpp) const {
		return FreeImage_FIFSupportsExportBPP(format, bpp) == TRUE;
	}
	bool supportsExportType(FREE_IMAGE_TYPE type) const {
		return FreeImage_FIFSupportsExportType(format, type) == TRUE;
	}
};

class FIP_API MimeFormat : public Format
{
public:
	MimeFormat(const std::string& mime) : Format(FreeImage_GetFIFFromMime(mime.c_str())) {}
};

class FIP_API FileFormat : public Format
{
public:
	FileFormat(const std::string& file) : Format(FreeImage_GetFileType(file.c_str(), 0)) {}
	FileFormat(const std::wstring& file) : Format(FreeImage_GetFileTypeU(file.c_str(), 0)) {}
};

class FIP_API FileTypeFormat : public Format
{
public:
#if 0
	FileTypeFormat(const std::string& file) : Format(FreeImage_GetFIFFromFilename(file.c_str())) {}
#endif
	FileTypeFormat(const std::wstring& file) : Format(FreeImage_GetFIFFromFilenameU(file.c_str())) {}
};


class FIP_API Information
{
public:
	virtual ~Information() = 0 {}
	virtual unsigned getWidth() const = 0;
	virtual unsigned getHeight() const = 0;
	virtual unsigned getBitsPerPixel() const = 0;
	...
	virtual FREE_IMAGE_TYPE getImageType() const = 0;
};

class FIP_API StaticInformation : public Information, public Object
{
private:
	unsigned _height;
	unsigned _width;
...
public:
	StaticInformation()
		...
		_type(FIT_BITMAP)
	{}
	StaticInformation(const Information& rhs)
		: _height(rhs.getHeight()),
		_width(rhs.getWidth()),
		_bpp(rhs.getBitsPerPixel()),
		_hr(rhs.getHorizontalResolution()),
		_vr(rhs.getVerticalResolution()),
		_fmt(rhs.getFormat()),
		_type(rhs.getImageType())
	{}
	virtual ~StaticInformation() {}

	StaticInformation& operator=(const Information &rhs) {
		_width = rhs.getWidth();
		_height = rhs.getHeight();
		_bpp = rhs.getBitsPerPixel();
		_hr = rhs.getHorizontalResolution();
	...
	virtual unsigned getBitsPerPixel() const {
		return _bpp;
	}
	...
	virtual FREE_IMAGE_TYPE getImageType() const {
		return _type;
	}
};
...
/** A class used to manage all photo related images and all image types used by the library.
	...
	Image encapsulates the FIBITMAP format. It relies on the FreeImage library, especially for 
	loading / saving images and for bit depth conversion.
	@version FreeImage 3
	@author Hervé Drolon
*/

class FIP_API Image : public Object, public Information
{
protected:
	/// DIB data
	FIBITMAP *_dib;
	Format _format;
	StaticInformation _origInfo;
	/// TRUE whenever the display need to be refreshed
	mutable bool _bHasChanged;

public:
	friend class MultiPage;
...
public:
	...
	*/
	Image(FREE_IMAGE_TYPE image_type = FIT_BITMAP, unsigned width = 0, unsigned height = 0, unsigned bpp = 0);
	/// Destructor
	virtual ~Image();
	/**
	Image allocator
	...
	*/
	Image& operator=(FIBITMAP *dib);


	/**
	@brief Copy a sub part of the current image and returns it as a Image object.
	...
	@see FreeImage_Copy
	*/
	bool copySubImage(Image& dst, int left, int top, int right, int bottom) const;
	...
	@see FreeImage_Paste
	*/
	bool pasteSubImage(Image& src, int left, int top, int alpha = 256);
	...
	*/
	static FREE_IMAGE_FORMAT identifyFIF(const std::string& lpszPathName);
#endif
	/**
	...
	*/
	static FREE_IMAGE_FORMAT identifyFIF(const std::wstring& lpszPathName);
#if 0
	/**
	...
	*/
	bool load(const std::string& lpszPathName, int flag = 0);
#endif
	/**
	UNICODE version of load (this function only works under WIN32 and does nothing on other OS)
	@see load
	*/
	bool load(const std::wstring& lpszPathName, int flag = 0);
#if 0
	/**
	...
	*/
	bool loadFromMemory(MemoryIO& memIO, int flag = 0);
	...
	*/
	bool save(const std::string& lpszPathName, int flag = 0) const;

	/**
	UNICODE version of save (this function only works under WIN32 and does nothing on other OS)
	@see save
	*/
	bool save(const std::wstring& lpszPathName, int flag = 0) const;
	...
	*/
	bool saveToMemory(FREE_IMAGE_FORMAT fif, MemoryIO& memIO, int flag = 0) const;
#endif
	//@}
	...
	Might be UNKNOWN when not loaded from an existing location
	*/
	virtual const Format& getFormat() const {
		return _format;
	}
	...
	*/
	...
	/**
	Returns the Information about the originally created/loaded image
	*/
	const Information& getOriginalInformation() const {
		return _origInfo;
	}
	...
	*/
	...
	//@{
	/**
	Returns a pointer to the bitmap's palette. If the bitmap doesn't have a palette, getPalette returns 0. 
	...
	*/
	bool getThumbnail(Image& image) const;
	...
	*/
	bool setThumbnail(const Image& image);
	...
	*/
	...
	@see FreeImage_SetTransparencysTable
	*/
	...
	*/
	...
	When the bkcolor parameter is 0, the background color is removed from the image.
	@return Returns TRUE if successful, FALSE otherwise. 
	...
	*/
	bool getChannel(Image& image, FREE_IMAGE_COLOR_CHANNEL channel) const;
	...
	*/
	bool setChannel(const Image& image, FREE_IMAGE_COLOR_CHANNEL channel);
	...
	*/
	bool splitChannels(Image& RedChannel, Image& GreenChannel, Image& BlueChannel) const;
	...
	*/
	bool combineChannels(const Image& red, const Image& green, const Image& blue);
	//@}
	...
	@return Returns the translated & rotated dib if successful, returns 0 otherwise
	...
	*/
	...
	@return Returns rotated dib if successful, returns 0 otherwise
	@see FreeImage_Rotate
	*/
	bool rotate(double angle, const void *bkcolor = 0);
	...
	*/
	bool rescale(unsigned new_width, unsigned new_height, FREE_IMAGE_FILTER filter,  bool keepAspect=false);
	...
	*/
	bool makeThumbnail(unsigned maxWidth, unsigned maxHeight = 0, bool convert = TRUE);
	//@}
	...
	When using the WinImage class, the image status is used to refresh the display. 
	...
	*/
	bool getMetadata(FREE_IMAGE_MDMODEL model, const std::string& key, Tag& tag) const;
	/**
	...
	*/
	bool setMetadata(FREE_IMAGE_MDMODEL model, const std::string& key, Tag& tag);
	//@}
...
/** A class designed for MS Windows (TM) platforms.
    ...
    WinImage provides methods used to :
	<ul>
...
class FIP_API WinImage : public Image
{
public:
	/**@name Creation & Destruction */
	//@{	
	...
	@see FreeImage_Clone
	*/
	WinImage& operator=(const Image& src);
	...
	@see FreeImage_Clone
	*/
	WinImage& operator=(const WinImage& src);

	WinImage& operator=(FIBITMAP *dib);
	...
	*/
	...
	The original image (located in the Image class) will not be affected by any of the operations 
	...
	*/
	void drawEx(HDC hDC, RECT& rcDest, bool useFileBkg = FALSE, RGBQUAD *appBkColor = 0, FIBITMAP *bg = 0) const;
...
/** Memory handle
	...
	MemoryIO is a class that allows you to load / save images from / to a memory stream.
	@version FreeImage 3
	@author Hervé Drolon
*/
class FIP_API MemoryIO : public Object
{
protected:
...
public :
	...
	*/
    MemoryIO(BYTE *data = 0, DWORD size_in_bytes = 0);
	...
	*/
	...
	@return Returns the loaded dib if successful, returns 0 otherwise
	...
	*/
	...
	@return Returns the loaded multi-page if successful, returns 0 otherwise
...
private:
	/// Disable copy
	MemoryIO(const MemoryIO& src);
	/// Disable copy
	MemoryIO& operator=(const MemoryIO& src);
...
/** Multi-page file stream
	...
	MultiPage encapsulates the multi-page API. It supports reading/writing 
	multi-page TIFF, ICO and GIF files. 
*/
class FIP_API MultiPage : public Object 
{
protected:
...
public:
	...
	*/
	MultiPage(bool keep_cache_in_memory = FALSE);
	...
	*/
	bool open(const std::string& lpszPathName, bool create_new, bool read_only, int flags = 0);
	...
	@see FreeImage_LoadMultiBitmapFromMemory
	*/
	bool open(MemoryIO& memIO, int flags = 0);
	...
	*/
	bool saveToMemory(FREE_IMAGE_FORMAT fif, MemoryIO& memIO, int flags = 0) const;
	...
	*/
	void insertPage(int page, Image& image);
	...
	*/
	...
	MultiPage mpage;
	// ...
	Image image;		// You must declare this before
	...
	@param page Page number
	@return Returns the page if successful, returns 0 otherwise
	...
	*/
	void unlockPage(Image& image, bool changed);

	/**
	Returns an array of page-numbers that are currently locked in memory. 
	When the pages parameter is 0, the size of the array is returned in the count variable. 
...
*/
class FIP_API Tag : public Object
{
protected:
...
public:
	...
	*/
	...
	/**
	Returns the tag description if available, returns 0 otherwise
	...
	*/
	FREE_IMAGE_MDTYPE getTagType() const;
	/**
	...
	*/
	std::string toString(FREE_IMAGE_MDMODEL model, char *Make = 0) const;
...
*/
class FIP_API MetadataFind : public Object
{
protected:
...
public:
	...
	*/
	bool findFirstMetadata(FREE_IMAGE_MDMODEL model, const Image& image, Tag& tag);
	/**
