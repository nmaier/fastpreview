...
FreeImage_SetTransparentIndex(FIBITMAP *dib, int index) {
	if (dib) {
		...
		if (count) {
      auto new_tt = std::make_unique<BYTE[]>(count);
			memset(new_tt.get(), 0xFF, count);
			if ((index >= 0) && (index < count)) {
				new_tt[index] = 0x00;
			}
			FreeImage_SetTransparencyTable(dib, new_tt.get(), count);
		}
	}
}
...
FreeImage_SetMetadata(FREE_IMAGE_MDMODEL model, FIBITMAP *dib, const char *key, FITAG *tag) {
	...
	if(key != NULL) {
		...
		if(tag) {
			...
			// delete existing tag
      if (tagmap && (*tagmap)[key]) {
        FreeImage_DeleteTag((*tagmap)[key]);
      }
